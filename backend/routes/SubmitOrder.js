...
async function GetEnabledOrDisabled(item_name) {
    ...
    try {
        const res = await client.query(
            `SELECT availability FROM menu_items WHERE item_name = '${item_name}';`
        );
...
async function GetPrice(option_ID, item_ID) {
    ...
    try {
        ...
        console.error(
            "Error executing query... option=" + option_ID + " item=" + item_ID
        );
    }
}
...
async function getOrderNumber(timestamp) {
    ...
    try {
        ...
        console.error(
            `Could not query database for order number... timestamp = ${timestamp}`
        );
    }
}
...
router.post("/submit-order", async (req, res) => {
    ...
    try {
        const type_ids = await Promise.all(
            types.map((type) => getOptionSerialNumber(type))
        );
        ...
        const item_ids = await Promise.all(
            items.map(
                async (sub_array) =>
                    await Promise.all(
                        sub_array.map(
                            async (item) => await getItemSerialNumber(item)
                        )
                    )
            )
        );
        ...
        const prices = await Promise.all(
            joint.map(async ([type, items]) =>
                Promise.all(
                    items.map(async (item) => await GetPrice(type, item))
                )
            )
        );
        ...
        for (let i = 0; i < item_ids.length; ++i) {
            ...
            for (let j = 0; j < current_set.length; ++j) {
                ...
                const query = `INSERT INTO sales_order_history_details (order_number, item_serial_number) VALUES (${order_number}, ${
                    current_item + 12
                });`;
                await pool.query(query);
            }
        ...
        res.json({
            message: "Order failed to submit successfully",
            _er: err.stack,
        });
    }
});
...
router.get("/total-price", async (req, res) => {
    try {
        ...
        // Calculate individual combo prices
        const comboPrices = prices.map((subArray) =>
            subArray
                .reduce((acc, price) => acc + parseFloat(price), 0)
                .toFixed(2)
        );
        ...
        const total_price = comboPrices
            .reduce((acc, price) => acc + parseFloat(price), 0)
            .toFixed(2);

        res.json({ total_price, comboPrices });
    } catch (err) {
        console.error("Error submitting order:", err.stack);
        res.status(500).json({
            message: "Order failed to submit successfully",
